            ...
            <el-table :data="tableData" style="width: 100%" class="box" ref="filterTable">
              ...
              <el-table-column align="center" label="编辑" width="300" fixed="right">
                <template slot-scope="scope">
                  ...
                  >编辑</el-button>
                  <el-button
                    v-if="scope.row.roleId == '2019080214444277130' && scope.row.isTg == '0'"
                    @click="Setmer(scope.row)"
                    style="float:right"
                    type="success"
                    size="small"
                  >设为推荐商家</el-button>
                  <el-button
                    v-if="scope.row.roleId == '2019080214444277130' && scope.row.isTg == '1'"
                    @click="Setmer(scope.row)"
                    style="float:right;"
                    size="small"
                  >取消推荐商家</el-button>
                ...
                <el-col width="100%" class="form">
                  <el-form ref="highForm" label-width="120px">
                    ...
                    <el-form-item label="联系人：">
                      <el-input
                        v-model="highForm.realName"
                        maxlength="4"
                        placeholder="请输入联系人(最多4个字符)"
                        clearable
                      ></el-input>
                    </el-form-item>
                    <el-form-item label="电话：">
                      <el-input
                        v-model="highForm.applayPhone"
                        maxlength="13"
                        placeholder="请输入电话"
                        clearable
                      ></el-input>
                    ...
                    <el-form-item label="简介：">
                      <el-input
                        v-model="highForm.remark"
                        placeholder="请输入简介(最多16个字符)"
                        maxlength="16"
                        clearable
                      ></el-input>
                    </el-form-item>
...
export default {
  ...
  data() {
    return {
      tableData: [],
      card: [
        { value: "2019080214444277130", label: "商家" },
        { value: "master", label: "大咖" },
        { value: "2019080214441887640", label: "品牌商" }
      ],
      total: null,
      currentPage: 1,
      pageSize: 10,
  ...
  methods: {
    ...
    Export() {
      ...
      let downloadUrl =
        highexport + "?token_=" + token_ + "&check_code=" + check_code;
      if (this.search.roleId) {
    ...
    sureEdit() {
      ...
      if (this.roleId == "2019080214444277130") {
        if (!this.highForm.tgUrl) {
          return this.$message.error("请输入商家推广链接");
        }
      }
    ...
    Delete(e) {
      this.$confirm(
        "该用户下的所有相关信息都将被删除，删除之后无法恢复，确认删除吗？",
        "提示",
        {
